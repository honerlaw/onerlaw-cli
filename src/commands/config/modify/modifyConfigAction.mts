...
import { modifyExistingConfiguration } from './modifyExistingConfiguration.mjs'

export async function modifyConfigAction(): Promise<void> {
  try {
    ...
    const config = await modifyExistingConfiguration(existingConfig)
    ...
    logError(`Failed to modify config: ${errorMessage}`)
    process.exit(1)
  }
}
